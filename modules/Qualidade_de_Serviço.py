...
def qualidade_de_servico(jms_user, token):
    ...
    # Realizar Export
    url = "https://gw.jtjms-br.com/networkmanagement/dispatchWaybill/export"
    ...
    payload = {
        ...
        "isFeeCostZero":0,
        "dispatchFinanceCode":"310000",
        "dispatchFinanceId":137,
        "countryId":"1"
        }
    ...
    # Lista de Download
    url = f"https://gw.jtjms-br.com/networkmanagement/ft/ftExport/pageBalance?current=1&size=100&total=2&operatorCode={jms_user}&jobName=&operatingStartTime={end_date}+00:00:00&operatingEndTime={end_date}+23:59:59"
    ...
    while True:
        ...
        if response.status_code == 200:
            try:
                ...
                if records:
                    ultimo_status = records[-1].get("statusType", None)

                    if ultimo_status != 2:
                        fileUrl = records[-1]["fileUrl"].replace("/", "%2F").replace(" ", "+")
                        break
    ...
    download_url = f"https://gw.jtjms-br.com/networkmanagement/file/downloadFile?path={fileUrl}" # Link do Download
